...
#lastmod: 2023-02-06T04:56:09-08:00
tags: []
description: "Analyzing a deconstucted Unity Android application."
draft: false
# cover:
...
Recently, I was interesting in comparing two Android games built with Unity. I was particularly interested in analyzing 
...
- adb
  ...
  - `adb pull` - Transfer the app files to your your computer for further analysis
...
For the purposes of this post, we are going take a look at Marvel Snap. The Snap download from the Play Store is under 
150mb; however, it does appear to download a small amount of data when launched and more data as you progress. 
Developers may desire their submitted app package to be under 150mb because it can be downloaded on a mobile data plan 
without prompting the user about the download size. The Apple App Store has a similar limit at 200mb.

The analysis conducted for this post performed on Windows with WSL2, Ubuntu, and ZSH. You may already have an Android 
development environment setup with `adb` available in your PATH variable. If not, here are some [general 
...
First, we enable developer mode on our Android device. The steps for doing this can vary by version of Android or by 
...
Bridge (`adb`). Getting `adb` to connect to an Android device on WSL2 requires some extra steps. If we were to run 
`adb devices` without additional WSL2 setup, we should see the following output and notice that no devices are listed.
...
We need install ADB on both Windows and WSL2. Here is how we install ADB on Windows:
...
the computer where the devices is connected. Change to the directory where we want to copy files to and run `adb pull 
...
Before we get too far, lets take a quick look at the contents of the files we transferred. For Snap, it should look like 
the following:
...
additional APK for each supported target architecture. We can see in the output above that the binary APK is called 
`split_config.arm64_v8a.apk`.
...
We can easily look at the binaries in `lib/` to to understand the extracted size of the files; however, I am more 
interested in the compressed size of binaries as it relates to App Store size limits and the 150mb threshold. We need to 
have a look inside binary APK and to do this we will use the `zipinfo` command. If you don't have `zipinfo` installed, 
you can install it on WSL2 with `sudo adb install zip unzip`. We could simply look at the size of the binary APK to 
understand the size of all binaries but I'm interested in looking at the size of individual binaries.
...
We can see in the last line the total uncompressed and compressed size of all the files. We also see the compression 
ratio of 67.8%. For each file, we can see the compressed and uncompressed size in bytes of each file. If you would 
rather see the compression ratio as percent instead of a size in bytes, you can change the arguments we used and run 
...
We are interested in app size so lets take a high level look at the distribution of large files in the APK. Another 
...
`Assets/StreamingAssets/`, and all the assets that were embedded as part of scenes and `Resources/` directories. Lets 
...
When you embed asset bundles into a Unity APK, you have a decision to make. Do you compress the asset bundle in the APK 
archive or do you leave them uncompressed? Compressing the bundles is in addition to the compression setting used to 
build the individual bundles. That is to say, you can double compress asset bundles. The contents of a bundle itself 
can be compressed and the section of the APK archive containing the bundle can also be compressed. 
...
out of the APK in order to read it into memory. As an example, in a game I worked on where we pre-loaded many bundles, 
compressing the bundles increased app startup time by at least a multiple of 4 on decent hardware. You can improve this 
experience by extracting the asset bundles and caching them outside of the APK. Unity has a builtin asset bundle cache 
...
does the fact that we see compression ratios in the 6th column. Interestingly and assuming this was deliberate choice, 
...
The compression ratio varies widely among the bundles. We can see values ranging from single digits all the way up to 
...
We already looked at asset bundles but what about all the assets references by embedded scenes and assets found inside 
...
looking at the contents and structure of the project. For my purposes, I was interesting in looking at how much font 
